...
SELECT @nCantidadClientes = (
        SELECT COUNT(A.nCodigoCliente)
        FROM Clientes A
)

SELECT @nCodigoCliente = RAND() * @nCantidadClientes + 1

SELECT 'Soy el Cliente seleccionado para la Comprobante', A.*
FROM Clientes A
...
SELECT @nCantidadProductos = (
        SELECT COUNT(A.nCodigoProducto)
        FROM Productos A
)

SELECT @nCodigoProducto = RAND() * @nCantidadProductos + 1

SELECT 'Soy el producto seleccionado para la Comprobante', A.*
FROM Productos A
...
SELECT @nCantidadTipoComprobante = (
        SELECT COUNT(A.nCodigoTipoComprobante)
        FROM TipoComprobante A
)

SELECT @nCodigoTipoComprobante = RAND() * @nCantidadTipoComprobante + 1

SELECT @cSerie = A.cSerieActual
FROM TipoComprobante A
WHERE A.nCodigoTipoComprobante = @nCodigoTipoComprobante

DECLARE @cNumeroInicial VARCHAR(10) -- Variable auxiliar

SELECT @cNumeroInicial = ISNULL((
        SELECT COUNT(A.nCodigoComprobante)
        FROM Comprobantes A
        WHERE A.cSerie = @cSerie
),0) + 1

SELECT @cNumero = RIGHT('00000000' + @cNumeroInicial, 8)
...
SELECT @nCodigoComprobante = SCOPE_IDENTITY()
...
SELECT @nPrecioUnitario = A.nPrecioUnitario
FROM Productos A
WHERE A.nCodigoProducto = @nCodigoProducto

SELECT @nUnidades = RAND() * 10 + 1

SELECT @nMontoTotal = @nPrecioUnitario * @nUnidades

SELECT @nCodigoComprobante, @nUnidades, @nMontoTotal
...
SELECT @nMontoTotalComprobante = (
        SELECT SUM(A.nMontoTotal)
        FROM DetalleComprobante A
        WHERE A.nCodigoComprobante = @nCodigoComprobante
)

UPDATE Comprobantes
SET nMontoTotal = @nMontoTotalComprobante
WHERE nCodigoComprobante = @nCodigoComprobante
